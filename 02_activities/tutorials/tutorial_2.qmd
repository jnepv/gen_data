...
# --logistic can be changed for --glm to run linear regression for continuous traits, or --linear for a simpler linear model without covariate adjustment.
```
...
#          cex.axis = 1.5,
#          col = c("lightblue", "lightslateblue"),
#         chrlabs=as.character(1:22),             # Manually set chromosome labels to 1, 2, ..., 22 and x-axis ticks to match
#         cex.axis=0.7,                           # Shrink the size
...
# --condition-list <document.txt> can be used to condition on multiple SNPs at once (instead of --condition)
```
