...
fam_data <- fread("./02_activities/data/gwa.A2.fam", header = FALSE)
num_samples <- nrow(fam_data)
num_samples
...
bim_data <- fread("./02_activities/data/gwa.A2.bim", header = FALSE)
num_snps <- nrow(bim_data)
num_snps

```

There are 306,102 SNPs in the dataset.
...
# Pruned dataset
plink --bfile ./02_activities/data/gwa.qc.A2 --extract ./02_activities/data/gwa.qc.A2.prune.in --make-bed --out ./02_activities/data/gwa.qc_pruned.A2

# Identify unrelated individuals
plink2 --bfile ./02_activities/data/gwa.qc_pruned.A2 --king-cutoff 0.0884 --out ./02_activities/data/gwa.qc_pruned.A2
...
plink2 --bfile ./02_activities/data/gwa.qc.A2 --keep ./02_activities/data/gwa.qc_pruned.A2.king.cutoff.in.id --make-bed --out ./02_activities/data/gwa.qc_unrelated.A2
...
setopt interactivecomments
# Pruned dataset with unrelated individuals
...
plink2 --bfile ./02_activities/data/gwa.qc_unrelated.A2 --ci 0.95 --linear --covar ./02_activities/data/gwa.A2_unrel_PCA.eigenvec --covar-name PC1,PC2,PC3 --adjust --out ./02_activities/data/gwa.qc_assoc.A2
...
assoc.A2 <- data.table::fread("./02_activities/data/gwa.qc_assoc.A2.PHENO1.glm.linear")
head(assoc.A2)

# Keep only the additive test (one row per SNP)
assoc.A2_add <- assoc.A2[TEST == "ADD"]

# qqman expects columns named CHR, BP, SNP, and P.
# In PLINK2 output they are #CHROM, POS, ID, and P.
setnames(assoc.A2_add, c("#CHROM","POS","ID"), c("CHR","BP","SNP"))

# Manhattan plot
png("./02_activities/data/manhattan.A2_plot.png", width = 1200, height = 800, res = 150)

manhattan(assoc.A2_add,
          chr = "CHR",
          bp  = "BP",
          snp = "SNP",
          p   = "P",
...
qq(assoc.A2_add$P, main = "Q-Q plot of GWAS p-values")
dev.off()
```
